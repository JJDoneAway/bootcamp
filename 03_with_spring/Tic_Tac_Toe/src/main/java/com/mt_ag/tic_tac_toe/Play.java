...
import com.mt_ag.tic_tac_toe.IBoard.VALIDATION;
...
public class Play {
	...
	public static void main(String[] args) {
		...
		while (board.checkStatus().equals(GAME_STATUS.RUNNING)) {
			...
			IPosition nextPos = validateInput(board, input);
			if (nextPos == null) {
				continue;
			}
			board.move(nextPos);
	...
	private static IPosition validateInput(IBoard board, String input) {
		// valid length
		if (input.length() < 3) {
			System.err.printf(
